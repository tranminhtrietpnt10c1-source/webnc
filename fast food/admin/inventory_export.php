    ...
    try {
        switch ($action) {
            ...
            case 'get_export_transactions':
                ...
                // Export transactions
                $exportSql = "SELECT od.quantity, o.order_date, o.order_code, u.full_name as customer_name, o.total_amount
...
<head>
    ...
    <title>Tra cứu xuất kho - Feane Restaurant</title>
...
<body>
    <div id="admin-page">
        ...
        <div class="main-content">
            ...
            <h2 class="mb-4">Tra cứu xuất kho</h2>
            ...
            <ul class="nav nav-tabs stock-tabs" id="stockTabs" role="tablist">
                ...
                <li class="nav-item" role="presentation">
                    <a class="nav-link active" href="inventory_export.php">Tra cứu xuất kho</a>
                </li>
            </ul>

            <!-- Tra cứu xuất kho -->
            <div class="search-section">
                <h5><i class="fas fa-arrow-up me-2"></i>Tra cứu xuất kho</h5>
                <form id="export-search-form">
                    <div class="row">
                        <div class="col-md-4 mb-3">
                            <label for="export-product" class="form-label">Chọn sản phẩm</label>
                            <select class="form-select" id="export-product" name="export-product">
                                <option value="">-- Chọn sản phẩm --</option>
                            </select>
                        </div>
                        <div class="col-md-3 mb-3">
                            <label for="export-date-from" class="form-label">Từ ngày</label>
                            <input type="date" class="form-control" id="export-date-from" name="export-date-from">
                        </div>
                        <div class="col-md-3 mb-3">
                            <label for="export-date-to" class="form-label">Đến ngày</label>
                            <input type="date" class="form-control" id="export-date-to" name="export-date-to">
                        </div>
                ...
                <div id="export-search-result" class="detail-search-result" style="display: none;">
                    <h5>Kết quả tra cứu xuất kho</h5>
                    <div class="row mb-4">
                        <div class="col-md-12">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered transaction-table">
                            ...
                            <tbody id="export-result-body"></tbody>
                        </table>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        $('#export-search-form').submit(function(e) {
            e.preventDefault();
            const productId = $('#export-product').val();
            const dateFrom = $('#export-date-from').val();
            const dateTo = $('#export-date-to').val();
            if (!productId) { alert('Vui lòng chọn sản phẩm'); return; }
            
            $.getJSON('inventory_export.php', { action: 'get_export_transactions', product_id: productId, date_from: dateFrom, date_to: dateTo }, function(res) {
                if (res.success) {
                    $('#total-export').text(res.total_export);
                    const tbody = $('#export-result-body');
                    tbody.empty();
                    if (res.transactions.length === 0) {
                        tbody.html('应用<td colspan="5" class="text-center">Không có giao dịch xuất kho</td>应用');
                    } else {
                        res.transactions.forEach(t => {
                            tbody.append(`
                                <tr>
                                    <td>${new Date(t.order_date).toLocaleDateString('vi-VN')}</td>
                                    <td>${t.order_code || '---'}</td>
                                    <td>${t.customer_name || 'Khách lẻ'}</td>
                                    <td class="text-center">${t.quantity}</td>
                                    <td class="text-end">${formatMoney(t.total_amount)}</td>
                                </tr>
                            `);
                        });
                    }
                    $('#export-search-result').show();
                } else {
                    alert('Lỗi: ' + res.error);
                }
            });
        ...
        $('#export-date-from').val(formatDate(thirtyDaysAgo));
        $('#export-date-to').val(formatDate(today));
